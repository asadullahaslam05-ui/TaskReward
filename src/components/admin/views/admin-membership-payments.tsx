...
type PaymentMembership = {
  id?: string;
  planId?: string;
  plan?: { id?: string; name?: string; monthlyFee?: number } | null;
  status?: string;
};
...
export function AdminMembershipPayments() {
  const queryClient = useQueryClient();
  const [statusFilter, setStatusFilter] = useState<string>("ALL");
  const [page, setPage] = useState(1);
  const [detailPayment, setDetailPayment] = useState<MembershipPayment | null>(null);
  const [screenshotPayment, setScreenshotPayment] = useState<MembershipPayment | null>(null);
  ...
  const queryPath = useMemo(() => {
    ...
    return `/api/supabase/admin/membership-payments?${params.toString()}`;
  }, [page, statusFilter]);

  const { data, isLoading, isFetching, isError } = useQuery<PaymentsResponse>({
    queryKey: ["admin-supabase-membership-payments", queryPath],
    queryFn: () => apiGet<PaymentsResponse>(queryPath),
    onError: (err: any) => toast.error(err?.message || "Failed to load membership payments"),
  });

  // Handle both `items` and `payments` response keys
  const payments: MembershipPayment[] = data?.items ?? data?.payments ?? [];
  ...
  const reviewMutation = useMutation({
    mutationFn: (vars: {
      ...
      apiPatch(`/api/supabase/admin/membership-payments/${vars.id}`, {
        action: vars.action,
    ...
    onSuccess: (_d, vars) => {
      toast.success(
        vars.action === "APPROVED"
          ? "Membership payment approved & membership extended"
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Review and approve pending membership plan payments.
          </p>
        </div>
        <Select
          value={statusFilter}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="p-4">
          ...
          <div className="text-xl font-bold mt-1 text-amber-600">{totalPending}</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs text-muted-foreground">Approved (page)</div>
          <div className="text-xl font-bold mt-1 text-emerald-600">{totalApproved}</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs text-muted-foreground">Rejected (page)</div>
          <div className="text-xl font-bold mt-1 text-red-600">{totalRejected}</div>
        </Card>
      </div>
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {isError ? (
          ...
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm font-medium">No membership payments found</p>
            <p className="text-xs text-muted-foreground mt-1">
              {statusFilter !== "ALL"
                ? `No ${statusFilter.toLowerCase()} membership payments.`
                  ...
                  {payments.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-muted-foreground text-xs">
                        {formatDateShort(p?.paymentDate || p?.createdAt || null)}
                      </TableCell>
                      <TableCell>
          ...
          {detailPayment && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Submitted {formatDate(detailPayment?.createdAt || null)}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <StatusBadge status={detailPayment?.status || "UNKNOWN"} />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <DetailRow
                    ...
                    label="Plan"
                    value={detailPayment.membership?.plan?.name || "—"}
                  />
                  <DetailRow
                    icon={<Wallet className="h-4 w-4" />}
                    label="Amount"
                    value={
                      typeof detailPayment.amount === "number" ? (
                        <CurrencyDisplay amount={detailPayment.amount} />
                      ) : (
                        "—"
                      )
                    }
                  ...
                  <DetailRow
                    ...
                    label="Payment Date"
                    value={formatDateShort(detailPayment?.paymentDate || null)}
                  />
                  {detailPayment?.reviewedAt && (
                    <DetailRow
                      icon={<Calendar className="h-4 w-4" />}
                      label="Reviewed At"
                      value={formatDate(detailPayment?.reviewedAt)}
                    />
                  )}
                </div>
                  ...
                  <div>
                    ...
                    >
                      <div className="relative aspect-video w-full overflow-hidden rounded-lg border bg-muted/50 hover:border-violet-300 transition-colors">
                        <img
                          src={detailPayment.screenshotUrl}
                          ...
                          onError={(e) => {
                            ...
                            if (parent) {
                              parent.innerHTML = `<div class="flex items-center justify-center h-full text-muted-foreground text-sm"><svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>Failed to load image</div>`;
                            }
                          }}
                        />
                      </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {reviewDialog.action === "APPROVED"
                ? "Approving will mark the payment as APPROVED, extend/activate the user's membership, and write a ledger entry."
                : "Rejecting will mark the payment as REJECTED. The user will be notified."}
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-3xl p-2" showCloseButton>
          <DialogHeader className="p-2">
            ...
            <DialogDescription className="text-xs">
              {screenshotPayment?.user?.fullName ||
                screenshotPayment?.user?.email ||
                "—"}{" "}
              • Txn: {screenshotPayment?.transactionId || "—"}
            </DialogDescription>
          </DialogHeader>
          {screenshotPayment && (
            <div className="overflow-auto max-h-[80vh]">
              {screenshotPayment.screenshotUrl ? (
                <img
                  src={screenshotPayment.screenshotUrl}
                  alt="Payment screenshot"
