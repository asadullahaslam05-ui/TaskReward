...
export function AdminPayments() {
  ...
  const { data, isLoading, isFetching, isError } = useQuery<PaymentsResponse>({
    ...
    onError: (err: any) => toast.error(err.message || "Failed to load payments"),
  });
  ...
  const reviewMutation = useMutation({
    mutationFn: (vars: { id: string; action: "APPROVE" | "REJECT"; adminNote: string }) =>
  ...
  const openReview = (payment: RegistrationPayment, action: "APPROVE" | "REJECT") => {
              ...
              <div className="space-y-4">
                ...
                {detailPayment.status === "PENDING" && (
                  <div className="flex gap-2 pt-2">
                    <Button
                      onClick={() => openReview(detailPayment, "APPROVE")}
                      className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white"
                    >
                    ...
                    <Button
                      variant="outline"
                      onClick={() => openReview(detailPayment, "REJECT")}
        ...
        <DialogContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="adminNote" className="text-xs">
              Admin Note {reviewDialog.action === "REJECT" && "(required)"}
            </Label>
            <Textarea
              id="adminNote"
              value={adminNote}
            ...
            <Button
              onClick={() => {
                if (reviewDialog.payment && reviewDialog.action) {
                  if (reviewDialog.action === "REJECT" && !adminNote.trim()) {
              ...
              disabled={
                ...
                (reviewDialog.action === "REJECT" && !adminNote.trim())
              }
              className={
                reviewDialog.action === "APPROVE"
