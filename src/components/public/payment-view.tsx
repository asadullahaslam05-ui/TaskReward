...
import { Wallet, Loader2, Upload, Copy, CheckCircle2, Clock, XCircle, LogOut, CreditCard, QrCode, AlertTriangle } from "lucide-react";
...
export function PaymentView() {
  ...
  // The registration fee MUST come from the Admin/Supabase configuration.
  // Never fall back to a hardcoded amount (e.g. 500). The client amount is
  // display information only; the server re-reads `registration.fee` from
  // site_settings and is the authoritative validator. If the configured fee
  // is missing/invalid, we surface a configuration error and disable payment
  // submission instead of silently using a wrong number.
  ...
  const [form, setForm] = useState({
    ...
    senderAccount: "",
    transactionId: "",
    amount: feeConfigured ? (registrationFee as number) : 0,
    paymentDate: "",
    screenshotPath: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!feeConfigured) {
      toast.error("Registration fee is not configured. Please contact support.");
      return;
    }

    setSubmitting(true);
    try {
      // `amount` is sent for display/reference only. The server re-reads the
      // canonical `registration.fee` from site_settings and validates against
      // it; a manipulated client amount is rejected. We send the configured
      // fee so the recorded amount matches the authoritative value.
      await apiPost("/api/supabase/registration-payments", {
        paymentMethodId: selectedMethod.id,
        senderName: form.senderName,
        senderAccount: form.senderAccount,
        transactionId: form.transactionId,
        amount: registrationFee as number,
                ...
                <div className="mt-3 flex items-center gap-2">
                  ...
                  {feeConfigured ? (
                    <Badge className="bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-400">
                      {settings?.currencySymbol || "Rs"} {registrationFee}
                    </Badge>
                  ) : (
                ...
                <Card className="p-6">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          type="number"
                          step="0.01"
                          value={feeConfigured ? registrationFee : ""}
                          disabled
                          readOnly
                          required
                        />
                    ...
                    <Button
                      type="submit"
                      className="w-full"
                      disabled={submitting || uploading || !feeConfigured}
                    >
                      {!feeConfigured ? (
                        <>
